...
const BRANCHES = {
  ...
  english: [
    ...
    { id: 'footprints', name: 'Footprints Without Feet' },
...
function goto(view){
  ...
  const map = { notesPicker:'/pick/notes', formulaPicker:'/pick/formulas', quizPicker:'/pick/quiz', soloPicker:'/pick/solo', createWar:'/pick/war', mcqPractice:'/pick/mcqpractice', vault:'/vault' };
...
function renderChBody(ch, tab){
  ...
  if(tab==='notes'){
    ...
    fetch('/api/'+state.subject+'/mcq/chapter/'+ch.id).then(r=>r.json()).then(items=>{
      body.innerHTML = items.map(mcqPracticeItemHTML).join('') || '<div class="empty">No MCQs loaded for this chapter yet.</div>';
    });
