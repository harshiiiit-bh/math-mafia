...
<script>
const state = { name: '', view: 'dashboard', selectedChapters: [], chapters: [], currentChapter: null, quiz: null, quizIdx: 0, quizScore: 0, testCode: null, testTimer: null, testStart: null, expandedChapter: null };
...
function enterApp(name){
  ...
  loadChapters().then(()=>{ renderDashboard(); initTicker(); });
}
...
function renderDashboard(){
  state.view='dashboard'; state.selectedChapters=[]; state.expandedChapter=null;
  ...
  main.innerHTML = \`
    ...
    <div class="ch-expand" id="chExpand"></div>
  \`;
  ...
  grid.innerHTML = state.chapters.map(c => {
    ...
    return \`
    <div class="ch-card clickable" data-id="${'$'}{c.id}" data-total="${'$'}{total}" onclick="toggleChapterCard(this,${'$'}{c.id})">
...
// ---------- Chapter content (Notes / Formulas / Practice Bank) ----------
...
// ---------- Dashboard inline chapter card (click to highlight + load in place) ----------
async function toggleChapterCard(el, id){
  const panel = document.getElementById('chExpand');
  if(state.expandedChapter === id){ closeChapterCard(); return; }
  document.querySelectorAll('.ch-card.clickable').forEach(c=>c.classList.remove('active'));
  el.classList.add('active');
  state.expandedChapter = id;
  const res = await fetch('/api/chapter/'+id);
  const ch = await res.json();
  panel.classList.add('show');
  panel.innerHTML = \`
    <div class="exp-head">
      <div><div class="num">CH ${'$'}{ch.chapter}</div><h3>${'$'}{esc(ch.title)}</h3></div>
      <button class="exp-close" onclick="closeChapterCard()">Close ✕</button>
    </div>
    <div class="tabs" id="expTabs">
      <button class="tab active" data-t="notes">📒 Notes</button>
      <button class="tab" data-t="formulas">🧾 Formulas</button>
      <button class="tab" data-t="exercises">📚 Practice Bank</button>
    </div>
    <div id="expBody"></div>
  \`;
  panel.querySelectorAll('#expTabs .tab').forEach(t=>{
    t.addEventListener('click', ()=>{
      panel.querySelectorAll('#expTabs .tab').forEach(x=>x.classList.remove('active'));
      t.classList.add('active');
      renderExpBody(ch, t.dataset.t);
    });
  });
  renderExpBody(ch, 'notes');
  panel.scrollIntoView({behavior:'smooth', block:'nearest'});
}
function closeChapterCard(){
  document.querySelectorAll('.ch-card.clickable').forEach(c=>c.classList.remove('active'));
  const panel = document.getElementById('chExpand');
  panel.classList.remove('show'); panel.innerHTML=''; state.expandedChapter = null;
}
function renderExpBody(ch, tab){
