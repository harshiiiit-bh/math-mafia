...
function pypGradablePool() { return pypPool.filter(q => q.gradable); }
...
app.post('/api/pyp/test/create', (req, res) => {
  const { paperId, chapterIds, count, creatorName, difficulty, timeLimitSeconds } = req.body;
  let qids, label;
  if (paperId) {
  ...
  db.attempts[code] = [];
  saveDB(db);
  res.json({ code, questions: publicQuestionsAny(qids), chapters: label, timeLimitSeconds: db.tests[code].timeLimitSeconds });
});
...
<script>
const state = { name: '', view: 'dashboard', selectedChapters: [], chapters: [], meta: null, currentChapter: null, quiz: null, quizIdx: 0, quizScore: 0, testCode: null, testTimer: null, testStart: null, timeLimitSeconds: null, runnerMode: null, warMeta: null, pypPapers: null, pypChapters: null };
...
async function renderPypPaperDetail(id){
  ...
  document.getElementById('pypSoloBtn').addEventListener('click', ()=>startPypSoloTest({paperId:id, timeLimitSeconds: gradable.length ? Math.max(600, gradable.length*90) : null}));
  document.getElementById('pypWarBtn').addEventListener('click', ()=>createPypGangWar({paperId:id, timeLimitSeconds: gradable.length ? Math.max(600, gradable.length*90) : null}));
}
...
async function renderPypTestSetup(mode, chapterIds){
  ...
  main.innerHTML = \`
    ...
    <div class="setup-box">
      ...
      <div class="count-row" id="pypMinutesRow" style="${'$'}{modeInfo.defTimer?'':'display:none;'}">
        <input type="number" id="pypMinutesInput" min="1" value="${'$'}{modeInfo.defMinutes}" style="width:90px; background:var(--bg); border:1.5px solid var(--card-border); color:var(--cream); padding:10px; border-radius:6px; font-size:15px;">
        <span class="val" style="width:auto; font-family:'IBM Plex Sans'; font-size:12.5px; color:var(--muted);">minutes for the whole test</span>
      </div>
    </div>
    <div style="display:flex; justify-content:flex-end;">
  ...
  document.getElementById('pypSetupGo').addEventListener('click', ()=>{
    ...
    const minutes = Math.max(1, parseInt(document.getElementById('pypMinutesInput').value,10) || modeInfo.defMinutes);
    const opts = { chapterIds, difficulty, count, timeLimitSeconds: noTimer ? null : minutes*60 };
...
async function finishGangWar(){
  ...
  main.innerHTML = \`
    ...
    <div class="section-title" style="margin-top:20px;">Breakdown</div>
    ${'$'}{state.quiz.map(q=>{ const b = byQid[q.qid]||{}; return \`<div class="ex-item"><div class="q">${'$'}{renderMedia(q.question)}</div><div class="feedback ${'$'}{b.correct?'correct':'wrong'}">${'$'}{b.correct?'✅ Correct':'❌ Your answer: '+esc(q.given||'(blank)')+' — Correct: '+renderMedia(b.correctAnswer||'')}<span class="time-tag">⏱ ${'$'}{q.timeSec!=null?q.timeSec+'s':'—'}</span></div></div>\`; }).join('')}
  \`;
}
